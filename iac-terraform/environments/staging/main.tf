...
# This file instantiates all modules for the staging environment
...
module "ingress" {
  ...
  external_controller_replicas       = 2 # Multiple replicas for staging
  external_controller_cpu_request    = "100m"
  external_controller_memory_request = "128Mi"
  external_controller_cpu_limit      = "200m"
  external_controller_memory_limit   = "256Mi"

  # Internal Controller Configuration (Private)
  internal_controller_replicas       = 1 # Single replica for staging
  internal_controller_cpu_request    = "50m"
  internal_controller_memory_request = "64Mi"
  internal_controller_cpu_limit      = "100m"
  internal_controller_memory_limit   = "128Mi"

  # Features
  enable_ssl_termination       = true # Enable SSL for staging
  enable_access_logs           = true
  enable_metrics               = true
  enable_prometheus_monitoring = true # Enable monitoring for staging
  enable_admission_webhook     = true
  enable_default_backend       = true

  # Security
  enable_waf    = false # Disable WAF for staging (cost savings)
  enable_shield = false # Disable Shield for staging (cost savings)
...
module "postgres" {
  ...
  performance_insights_enabled          = true # Enable for staging
  performance_insights_retention_period = 7

  # Monitoring
  monitoring_interval = 60 # Enable enhanced monitoring for staging
  ...
  multi_az            = false # Disable for staging (cost savings)
  deletion_protection = true  # Enable for staging

  # One read replica for staging
  read_replicas = {
    "read-1" = {
